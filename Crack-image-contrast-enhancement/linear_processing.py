...
def linear_hist_equalization(img):
    ...
    dark_scale = 0.5 * new_peak_value / 128
    bright_scale = 2.5 * (255 - new_peak_value) / 128

    mapping = np.zeros(256, dtype=np.float32)
    for i in range(256):
        if i < new_peak_value:  # Dark region enhancement
            mapping[i] = i * dark_scale
        else:  # Bright region enhancement
            mapping[i] = new_peak_value + (i - new_peak_value) * bright_scale
